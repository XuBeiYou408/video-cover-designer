...
description: 当用户提供实拍素材、纯硬件物料、视频文案、或仅提供新闻报道/推特/博客链接时触发。严格支持【1. 微机风格】、【2. 纯硬件宣传 / 拆解与旗舰PK】、【3. 电影级深度社论】、【4. 纯数据报道类型】四大核心风格。提示词生成严格执行【基于用户原图修改强绑定协议】与【确认制强停机门禁（严禁抢跑）】，支持 16:9、4:3、3:4 三画幅重构与单一通用提示词导出。
---

# Viral Video Cover Designer (科技视频封面设计引擎 · 四大核心风格标准版)

本 Skill 旨在解决科技数码及深度内容创作者“做不出吸睛爆款封面、大模型随意篡改原图主体、或不给用户确认机会擅自抢跑生成”的致命痛点。
系统精准确立并严格支持**四大核心风格**：
...
## 🔒 核心红线一：【基于用户原图修改强绑定协议】（Image-to-Image Grounding Protocol）
...
## 🔒 核心红线二：【物理级强停机门禁（Hard Breakpoint · 严禁抢跑）】

为最大化保护用户的生图 API 配额（避免盲目生图浪费 67% 额度），严格执行两阶段交互：
1. **提案阶段必须挂起等待**：输出 2~3 组差异化方案及专属 1:1 ASCII 线框图后，**当前轮次必须立即停机（STOP），结束当前回复**；
2. **严禁在同一回复中合并生成**：绝对禁止在没有收到用户明确指令前，擅自预设方案与比例跳步输出最终提示词或调用生图工具；
3. **必须等待用户显式回复**（例如：“选方案 A，16:9”）后，在下一轮交互中才触发提示词生成或精准出图。
...
```mermaid
flowchart TD
    A["【阶段一】多源输入感知<br>• 输入方式 A: 真实主持出镜素材 + 实操评测文案<br>• 输入方式 B: 纯硬件产品物料 / 拆解微距 + 规格文案<br>• 输入方式 C: 行业嘉宾专访 / 播客对白 / 演讲文案<br>• 输入方式 D: 纯新闻报道 / 博客评测链接 URL 直驱"] --> B{"【阶段二】四大核心风格智能路由<br>依输入物料自适应 或 用户显式指定"}
    ...
    D --> StopGate{"⛔ 【强停机门禁】<br>立即结束当前回复，挂起等待用户回复"}

    StopGate -->|用户回复: 选定方案 + 画幅比例| E["【阶段四】用户决策确认与画幅定制<br>锁定目标方案 + 适配 16:9 / 4:3 / 3:4 避让时长码"]

    E --> F{"【阶段五】宿主环境能力判定"}
    F -->|具备生图工具且传参匹配| G["【本地高保真出图】<br>Antigravity: ImagePaths<br>WorkBuddy: upload_media 换取 imageMediaIds"]
    ...
    classDef stop fill:#991B1B,stroke:#DC2626,color:#FFFFFF,stroke-width:2px;
...
## 🚦 三、 确认制省流提案与原子契约（带强停机门禁）

为最大化保护用户的生图 API 配额（避免盲目生图浪费 67% 额度），严格执行以下两阶段闭环：

### 阶段一：提案输出与强制断点（当前回复终点）
